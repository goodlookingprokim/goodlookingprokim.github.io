...
버전: 1.3 (2026-07-12) · 이 파일은 4060 저장소 루트에 두고, 갤러리 자료를 접수·게시·수정하는 모든 작업 전에 반드시 읽는다.
...
## 7. 독자 안내 문구 (모든 갤러리 항목에 필수)

모든 갤러리 항목 페이지 하단에는 아래 팁 콜아웃을 그대로 넣는다. 원문 자료의 스크립트 로딩 경합으로 전체 화면이 간혹 비어 보이는 문제(원문 무수정 원칙상 파일 내부는 고치지 않음)와, 자료별 색 대비 차이를 독자가 스스로 해결할 수 있게 돕는 안내다.
...
갤러리 인덱스와 갤러리 허브(`galleries.md`)에는 "편하게 보는 팁" `cta-panel`(라이트/다크 전환 + 새로고침 안내)을 유지한다. 2026-07-12에 홈·허브·인덱스 3곳·항목 5곳에 배치 완료.

## 8. 4060 가독성 기본값 (갤러리 외 전체 적용)
...
- 1.3 (2026-07-12): 독자 안내 문구 장 신설 — 전체 화면 빈 화면(새로고침 안내)과 라이트/다크 전환 팁을 모든 갤러리 항목에 필수화.
