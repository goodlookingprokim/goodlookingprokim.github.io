...
여기서 마지막 표는 단순 참고자료가 아니다. 본문에서 읽은 흐름을 실제 링크로 다시 따라가게 만드는 실행판이다. 그래서 아래 순서대로 보는 편이 좋다.

### 1단계: 지금 바로 전달 가능한 링크

이미 공개 확인이 끝난 항목은 지금 외부 공유나 후속 정리에 바로 써도 되는 축이다.
...
이 네 개는 <strong>안전한 출발점</strong>이다.

### 2단계: 흐름 파악용으로 먼저 읽을 링크

`legacy_needs_validation` 항목은 최종 공개 검증은 덜 끝났지만, 방향을 읽기엔 충분히 좋다. Auto Research, Claude Skill, 무료 GPU 음성 생성, 공공데이터, book-to-skill, claude-scaffold 같은 항목이 여기에 해당한다.

즉 이 그룹은 “지금 당장 재배포용”이라기보다, <strong>최근 변화가 어디로 가는지 읽는 참고 축</strong>으로 쓰는 편이 맞다.

### 3단계: 다음 검증 큐로 남겨둘 링크

`unresolved`는 가치가 낮다는 뜻이 아니다. 자료 자체는 좋아도 공개 링크가 아직 완전히 확인되지 않았기 때문에, <strong>지금은 보류 자산</strong>으로 관리해야 한다는 뜻이다. 이 구간을 남겨두는 습관이 있어야 다음 큐레이션에서 다시 이어붙일 수 있다.

## 왜 이번 묶음에서 검증 상태도 함께 봐야 하나

이번 파일이 흥미로운 이유는, 링크 분석에서 얻은 인사이트와 함께 <strong>링크 자산의 사용 가능 상태</strong>도 같이 드러난다는 점이다. 다만 이건 글의 주인공이 아니라 보조 정보여야 한다.
...
즉 이번 묶음은 “40개 추천 링크 모음”이라기보다, <strong>최근 흐름을 읽으면서 동시에 어떤 링크가 실제 자산으로 쓸 준비가 됐는지 분류한 작업판</strong>에 가깝다. 그래서 마지막 표의 원본 링크와 LilysAI 공개 링크를 함께 남겨두는 것이 의미가 있다.

## 다음에 바로 쓸 편집 기준

- 첫 5문단 안에서 이번 묶음의 핵심 흐름이 보여야 한다.
- 카테고리는 단순 예시 나열이 아니라, 왜 이 링크들이 한 방향으로 읽히는지까지 설명해야 한다.
- 마지막 링크 표는 부록이 아니라 본문 결론의 실행판처럼 연결돼야 한다.
- 검증 상태는 인사이트를 보강해야지, 인사이트를 대신하면 안 된다.
